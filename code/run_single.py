...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    if args.task == 'ner' or args.task == 'chunking':
        results = {
            "loss": eval_loss,
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            mode, args.task, list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length)
        ),
    )
    ...
    if mode == 'train':
        features = features[:args.train_sent_num]
...
def main():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="The number of training sentences in the task"
    )
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "{}.txt".format('test_' + args.task)), "r") as f:
                example_id = 0
                for line in f:
