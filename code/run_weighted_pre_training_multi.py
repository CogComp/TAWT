...
from weighted_training_basics import get_task_weights_gradients_multi_pre
...
def auxiliary_train(args, train_auxiliary_dataset_list, train_main_dataset, model, tokenizer, auxiliary_labels_list,
                    main_labels, pad_token_label_id, auxiliary_task_list, main_task):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
    ...
    for train_auxiliary_dataset in train_auxiliary_dataset_list:
        ...
        auxiliary_train_sampler_list.append(auxiliary_train_sampler)
    ...
    for auxiliary_train_dataloader in auxiliary_train_dataloader_list:
        for i, auxiliary_batch in enumerate(auxiliary_train_dataloader):
            train_dataloader_auxiliary.append(auxiliary_batch)
    random.shuffle(train_dataloader_auxiliary)
    train_dataloader_main = []
    for i, main_batch in enumerate(main_train_dataloader):
        train_dataloader_main.append(main_batch)
    random.shuffle(train_dataloader_main)

    total_batch_num = len(train_dataloader_auxiliary)

    if args.max_steps > 0:
        t_total = args.max_steps
        args.num_train_epochs = args.max_steps // (total_batch_num // args.gradient_accumulation_steps) + 1
    else:
        t_total = total_batch_num // args.gradient_accumulation_steps * args.num_train_epochs
    ...
    if os.path.exists(args.model_name_or_path):
        ...
        try:
            ...
            global_step = 0
        epochs_trained = global_step // (total_batch_num // args.gradient_accumulation_steps)
        steps_trained_in_current_epoch = global_step % (total_batch_num // args.gradient_accumulation_steps)
    ...
    set_seed(args)  # Added here for reproductibility
    task_weights = [1 / len(auxiliary_task_list)] * len(auxiliary_task_list)
    for _ in train_iterator:
        print('task weights', task_weights)
        # training on the auxiliary datasets
        random.shuffle(train_dataloader_auxiliary)
        epoch_iterator_auxiliary = tqdm(train_dataloader_auxiliary,
                                        desc="Iteration", disable=args.local_rank not in [-1, 0])
        for step, batch in enumerate(epoch_iterator_auxiliary):
            # Skip past any already trained steps if resuming training
            if steps_trained_in_current_epoch > 0:
                steps_trained_in_current_epoch -= 1
                continue
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    # Save model checkpoint
                    output_dir = os.path.join(args.output_dir, "pre-train-auxiliary-checkpoint-{}".format(global_step))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = (
                        model.module if hasattr(model, "module") else model
            ...
            if args.max_steps > 0 and global_step > args.max_steps:
                epoch_iterator_auxiliary.close()
                break
        # training on the main dataset
        copy_args = copy.deepcopy(args)
        copy_args.num_train_epochs = 1.0
        main_model = copy.deepcopy(model)
        for param in main_model.bert.parameters():
            param.requires_grad = False
        main_train(copy_args, copy.deepcopy(train_main_dataset), main_model, copy.deepcopy(tokenizer),
                   copy.deepcopy(main_labels), copy.deepcopy(pad_token_label_id), copy.deepcopy(main_task))
        for param in main_model.bert.parameters():
            param.requires_grad = True
        # update weights
        print('task weights penalty', -entropy(task_weights))
        auxiliary_train_dataloader_list = []
        for x in range(len(auxiliary_task_list)):
            auxiliary_train_dataloader = DataLoader(train_auxiliary_dataset_list[x],
                                                    sampler=auxiliary_train_sampler_list[x],
                                                    batch_size=args.gradient_sampling_batch_size)
            auxiliary_train_dataloader_list.append(auxiliary_train_dataloader)
        main_train_dataloader = DataLoader(train_main_dataset, sampler=main_train_sampler,
                                           batch_size=args.gradient_sampling_batch_size)
        task_weights_gradients = get_task_weights_gradients_multi_pre(main_model, auxiliary_train_dataloader_list,
                                                                      main_train_dataloader, args)
        exp_ratio = np.exp(- args.weight_lr * task_weights_gradients)
        new_task_weights = np.array(task_weights) * exp_ratio
        new_task_weights /= np.sum(new_task_weights)
        task_weights = list(new_task_weights)
        if args.max_steps > 0 and global_step > args.max_steps:
            train_iterator.close()
            break
    if args.local_rank in [-1, 0]:
...
def train(args, train_auxiliary_dataset_list, train_main_dataset, model, tokenizer, auxiliary_labels_list, main_labels,
    ...
    if args.pre_train_only:
        ...
        return auxiliary_train(args, train_auxiliary_dataset_list, train_main_dataset, model, tokenizer,
                               auxiliary_labels_list, main_labels, pad_token_label_id, auxiliary_task_list, main_task)
    else:
        # pre-training on auxiliary datasets
        print('pre-training on auxiliary tasks')
        auxiliary_train(args, train_auxiliary_dataset_list, train_main_dataset, model, tokenizer,
                        auxiliary_labels_list, main_labels, pad_token_label_id, auxiliary_task_list, main_task)
...
def main():
    ...
    parser.add_argument("--weight_lr", default=1.0, type=float, help="The learning rate for mirror descent.")
    parser.add_argument("--normalized_gradients", action='store_true', default=False,
                        help="Normalize the weight gradients.")
    parser.add_argument("--gradient_sampling_batch_size", default=50, type=int, help="gradient sampling batch size")
    parser.add_argument("--inv_constant", default=0.0, type=float, help="The inv constant for approximate Hessian.")
