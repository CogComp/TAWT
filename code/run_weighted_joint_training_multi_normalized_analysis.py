...
from weighted_training_basics import get_task_weights_gradients_multi
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, task):
    ...
    if mode == 'train':
        if task == args.main_task:
            features = features[:args.main_task_train_sent_num]
        else:
            features = features[:int(args.main_task_train_sent_num * args.auxiliary_main_ratio)]
...
def train(args, train_auxiliary_dataset_list, train_main_dataset, model, tokenizer, auxiliary_labels_list, main_labels,
    ...
    total_task_num = len(auxiliary_task_list) + 1
    task_weights = [1 / args.auxiliary_main_ratio] * len(auxiliary_task_list)
    task_weights.append(1)
    task_weights = np.array(task_weights) / (len(auxiliary_task_list) / args.auxiliary_main_ratio + 1)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = loss * task_weights[task_id] * total_task_num
        ...
        main_train_dataloader = DataLoader(train_main_dataset, sampler=main_train_sampler,
                                           batch_size=args.gradient_sampling_batch_size)
        task_weights_gradients = get_task_weights_gradients_multi(model, auxiliary_train_dataloader_list,
...
def main():
    ...
    parser.add_argument(
        "--auxiliary_main_ratio",
        default=1.0,
        type=float,
        help="The ratio between the size of the auxiliary dataset and the size of the main dataset."
    )
